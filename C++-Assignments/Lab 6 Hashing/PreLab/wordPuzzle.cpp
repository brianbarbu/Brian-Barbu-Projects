...
int main(int argc, char** argv) {
	...
	string place;
	ifstream current(key.c_str());
	while(getline(current, place)){
		number++;
	}
	current.close();
	
	hashTable *created = new hashTable(number);

	current.open(key.c_str());
	while(getline(current, place)){
		if(place.length() >=3){
			(*created).insert(place);
		}
	}
	current.close();
	current.open(search.c_str());
    ...
    if(!readInGrid (search.c_str(), rows, cols)){
	...
	for ( int b = 0; b < cols; b++ ){
	    for ( int c = 0; c < 8; c++ ){
		for ( int d = 3; d < 23; d++){
		    string select(getWordInGrid(a,b,c,d,rows,cols));
		    if(d > select.length()){
			break;
		    }
		    if((*created).isFound(select)){
			count++;
		    	if(c==0){
				cardinal = "N";
		    	}
    ...
    cout << "Found all words in " << clock.getTime() << " seconds"<<endl;
